...
from transwraps.web import get,post,ctx, view,interceptor,seeother,notfound
from apis import api,APIError,APIValueError,APIPermissionError,APIResourceNotFound, Page
...
_COOKIE_NAME='aweseession'
_COOKIE_KEY=config.session.secret


def make_signed_cookie(id, password, max_age):
    '''创建由id、cookie存活时间和seesion所组成成的cookie'''
...
_RE_MD5= re.compile(r'^[a-z0-9]{32}$')
...
def register_user():
    ...
    user = Users(name=name, email=email, password=password,image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email).hexdigest())
    user.insert()
