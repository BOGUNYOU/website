...
class InterField(Field):
    ...
    def __init__(self,**kwargs):
        ...
        if not 'ddl' in kwargs:
            kwargs['ddl']='bigint'
        super(InterField, self).__init__(**kwargs)
...
class BoolenField(Field):
    def __init__(self,**kwargs):
        ...
        if not 'ddl' in kwargs:
            kwargs['ddl']= 'bool'
        super(BoolenField, self).__init__(**kwargs)
...
def _gen_sql(table_name, mappings):
    ...
    for f in sorted(mappings.values(),lambda x, y: cmp(x._order,y._order)):
        if not hasattr(f, 'ddl'):
            raise StandardError('not ddl in "%s"'% f)
        ddl = f.ddl
        nullable = f.nullable
        if f.primary_key:
            pk = f.name
        ...
        for k, v in attrs.iteritems():
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    ...
                    if v.updatable:
                        logging.warning('NOTE: change primary key to no-updatable')
                        v.updatable = False
                    if v.nullable:
...
class Model(dict):
    ...
    def delete(self):
        ...
        return self
    def delet_all(self):
        db.update('delete from %s' % self.__table__)
        logging.warning('delete all')
        return self
...
if __name__ == '__main__':
    ...
    db.create_engine('root', 'znbxd@1992', 'test')
